...
import { descargarPresupuestoPdf } from "@/app/actions";

// Descarga el PDF del presupuesto de un clic (generado en el servidor). No usa
// la impresión del navegador, así que sale siempre igual y no se rompe.
export function DescargarPdfBtn({
  ...
  async function descargar() {
    ...
    setError(null);
    try {
      const res = await descargarPresupuestoPdf(oportunidadId, versionId ?? null);
      if (res.error || !res.base64) {
        setError(res.error ?? "No se pudo generar el PDF.");
        return;
      }
      // base64 → blob → descarga automática.
      ...
      a.href = url;
      a.download = res.filename ?? "presupuesto.pdf";
