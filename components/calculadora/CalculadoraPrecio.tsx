      ...
      {verParams && (
        <div className="space-y-3 rounded-md border-hair border-border bg-beige-light/50 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <NumInput label="Corporativo: verde desde" value={cfg.margenesPorTipo?.corporativo?.verde ?? 15} onChange={(v) => setCfg({ ...cfg, margenesPorTipo: { ...cfg.margenesPorTipo, corporativo: { ...(cfg.margenesPorTipo?.corporativo ?? { verde: 15, ideal: 30 }), verde: v } } })} sufijo="%" hint="Banda propia de los corporativos (pisa la de temporada): aceptan margen bajo porque amortizan estructura." />
            <NumInput label="Corporativo: ideal" value={cfg.margenesPorTipo?.corporativo?.ideal ?? 30} onChange={(v) => setCfg({ ...cfg, margenesPorTipo: { ...cfg.margenesPorTipo, corporativo: { ...(cfg.margenesPorTipo?.corporativo ?? { verde: 15, ideal: 30 }), ideal: v } } })} sufijo="%" />
